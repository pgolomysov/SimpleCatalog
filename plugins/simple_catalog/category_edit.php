...
if ($action == 'simple_catalog_category_edit')
{
	$category = simple_catalog::getCategory((int)$_GET['category_id'],(int)$_GET['catalog_id'])->config;
}
...
//if we save catalog
  ...
  if (!isset($msg)) {
  	...
  	if ($action == 'simple_catalog_category_edit')
	{
		$result = simple_catalog::saveCategory((int)$_GET['category_id'], (int)$_GET['catalog_id'], $_POST['post-name'],$_POST['post-description'],$url);
	}
	
	if ($action == 'simple_catalog_add_category')
	{
		$result = simple_catalog::createCategory($_POST['post-name'],$_POST['post-description'],$url,(int)$_GET['catalog_id']);
	}
...
<h3 class="floated" style="float:left"><?php i18n('simple_catalog/NEW_CATEGORY'); ?></h3>

<form method="post">
	<input type="text" class="text" id="post-name" name="post-name" value="<?php echo $category->name[0]; ?>"/><br />
	<?php i18n('simple_catalog/DESCRIPTION')?>:
	<textarea class="" cols=1 rows=1 id="post-content" name="post-description" style=""><?php echo $category->description[0]; ?></textarea><br />
	URL<br />
	<input type="text" class="text" id="post-url" name="post-url" value="<?php echo $category->url[0]; ?>"/><br /><br />
